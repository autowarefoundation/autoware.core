...
TEST(PlanningTestManager, CommunicationTest)
{
  ...
  test_manager->testWithBehaviorGoalOnLeftSide(test_target_node, "behavior_goal_on_left_side_route_for_test");  
  EXPECT_GE(test_manager->getReceivedTopicNum(), 1);
  ...
  test_manager->testWithAbnormalPathWithLaneId(test_target_node, "abnormal_path_with_lane_id_for_test");  
  EXPECT_GE(test_manager->getReceivedTopicNum(), 1);
