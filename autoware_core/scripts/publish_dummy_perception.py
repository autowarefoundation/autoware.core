...
class DummyPerceptionPublisher(Node):
    def __init__(self):
        ...
        self.pointcloud_publisher = self.create_publisher(PointCloud2, '/perception/obstacle_segmentation/pointcloud', 10)
        self.pointcloud_timer = self.create_timer(1.0, self.publish_dummy_perception)

        # Publisher for PredictedObjects
        self.objects_publisher = self.create_publisher(PredictedObjects, '/perception/object_recognition/objects', 10)
